...
PRICES = {"jev": (0.042, 0.0), "haiku": (1.10, 5.50)}
...
out = {}
for task in TASKS:
    jev = sorted(load(task, "jev"), key=lambda r: r["i"])
    llm = sorted(load(task, "haiku"), key=lambda r: r["i"])
    print(f"== {task}")
    base_acc = sum(r["correct"] for r in llm) / len(llm)
    base_cost = sum(cost(r, "haiku") for r in llm) / len(llm) * 1000
    base_p50 = statistics.median(r["lat"] for r in llm)
    print(f"  haiku-only        acc={base_acc:.3f}  $/1k={base_cost:.3f}  p50={base_p50:.3f}s")
    rows = []
    for th in (0.5, 0.7, 0.8, 0.9):
        correct, costs, lats, esc = [], [], [], 0
        for j, h in zip(jev, llm):
            take = j["conf"] is not None and j["conf"] >= th
            esc += not take
            correct.append(float(j["correct"] if take else h["correct"]))
            costs.append(cost(j, "jev") + (0 if take else cost(h, "haiku")))
            lats.append(j["lat"] + (0 if take else h["lat"]))
        acc = sum(correct) / len(correct)
        lo, hi = boot(correct)
        diff = [c - float(h["correct"]) for c, h in zip(correct, llm)]
        dlo, dhi = boot(diff)
        row = {
            "min_conf": th,
        ...
        print(
            ...
            f"vs-haiku={row['vs_haiku']:+.3f} [{dlo:+.3f},{dhi:+.3f}]  $/1k={row['usd_per_1k']}  "
